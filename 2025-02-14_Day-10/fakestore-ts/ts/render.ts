...
const renderCartItems = async (data: Cart[]) => {
  ...
  const itemPriceList = <NodeListOf<HTMLElement>>(
    document.querySelectorAll(".item-price")
  );
  const totalPrice = <HTMLElement>document.querySelector(".total-price");
  totalPrice.textContent = `$${Array.from(itemPriceList)
    .map((itemPrice: HTMLElement) =>
      Number(itemPrice.textContent?.replace("$", ""))
    )
    .reduce((f, s) => f + s)}`;
};
